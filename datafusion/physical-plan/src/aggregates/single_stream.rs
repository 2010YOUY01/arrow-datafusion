...
/// Hash aggregation can execute in a single stage when raw input rows are
/// aggregated directly into final aggregate values. This stream implements that
/// single stage for both `Single` and `SinglePartitioned` aggregate modes.
///
/// # Example
///
...
/// Input: raw rows
/// Output: final aggregate values for all groups (for example, `AVG(x)` is
/// calculated from the accumulated `SUM(x)` and `COUNT(x)` state)
pub(crate) struct SingleHashAggregateStream {
    /// Output schema: group columns followed by final aggregate value columns.
    schema: SchemaRef,

    /// Input batches containing raw rows.
...
/// States for single-stage hash aggregation processing.
...
impl SingleHashAggregateStream {
    pub fn new(
        ...
        assert_or_internal_err!(
            hash_table.mode() == AggregateTableMode::Single,
            "SingleHashAggregateStream expected single aggregate hash table"
        );
    ...
    /// Handle ReadingInput state - aggregate raw input batches into the hash table.
    ///
...
impl Stream for SingleHashAggregateStream {
    ...
    /// Entry point for the single-stage hash aggregate state machine.
    ///
    ...
    /// (start)
    ///   -> ReadingInput
    ///      The stream starts by polling raw input and aggregating those rows
    ///      into the single-stage hash table.
    ///
    /// ReadingInput
    ///   -> ReadingInput
